...
if ( ! function_exists( 'rpgc_checkout_field' ) ) {
	 ...
	 * @return void
	 */
	function rpgc_checkout_field() {
		global $woocommerce;
		?>
		<form class="cart_giftcard" method="post" >
			<label for="giftcard_code"><?php _e( 'Gift Card', 'woocommerce' ); ?>:</label> <input type="text" name="giftcard_code" class="input-text" id="giftcard_code" value="" placeholder="<?php _e( 'Gift card code', 'woocommerce' ); ?>" /> <input type="submit" class="button" name="apply_giftcard" value="<?php _e( 'Apply Gift Card', 'woocommerce' ); ?>" />
		</form>
		<script>
			jQuery(document).ready(function($) {
					/* AJAX Coupon Form Submission */
					$('form.cart_giftcard').submit( function() {
						var $form = $(this);
	...
	add_action( 'woocommerce_cart_coupon', 'rpgc_checkout_field', 10 );
}
...
/*
		// Add Discount
		if ( ! empty( $_POST['apply_giftcard'] ) && ! empty( $_POST['giftcard_code'] ) ) {
			add_giftcard( sanitize_text_field( $_POST['giftcard_code'] ) );
		}

		// Remove Coupon Codes
		elseif ( isset( $_GET['remove_coupon'] ) ) {

			remove_giftcard( wc_clean( $_GET['remove_giftcard'] ) );
...
*/
	...
	function remove_giftcard( $giftcard_code ) {
			// Coupons are globally disabled
			if ( ! $this->coupons_enabled() )
				return false;

			// Get the coupon
			$giftcard_code  = apply_filters( 'woocommerce_giftcard_code', $giftcard_code );
			$position     = array_search( $giftcard_code, $this->applied_giftcard );

			if ( $position !== false )
				unset( $this->applied_giftcard[ $position ] );

			WC()->session->set( 'applied_giftcard', $this->applied_giftcard );
